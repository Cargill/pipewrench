...
CREATE EXTERNAL TABLE IF NOT EXISTS {{ table.destination.name.replace('/','_').replace('.','_') }} (
...
{%- endfor %})
STORED AS PARQUET
LOCATION '{{ conf.result_database.path }}/{{ table.destination.name.replace('/','_').replace('.','_') }}/mod_val=1/'
TBLPROPERTIES(
