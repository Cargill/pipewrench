...
${impala%?}q "INSERT OVERWRITE TABLE {{ conf.result_database.name }}.{{ table.destination.name }}_parquet PARTITION (mod_val=$val) SELECT * FROM {{ conf.staging_database.name }}.{{ table.destination.name }}_avro;"
